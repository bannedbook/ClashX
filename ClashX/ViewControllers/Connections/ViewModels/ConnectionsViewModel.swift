...
class ConnectionsViewModel {
    ...
    private func updateForActiveMode(snapShot: ClashConnectionSnapShot) {
        ...
        currentHosts = Array(Set(currentConnections.map {$0.metadata.displayHost}))
        currentSourceIPs = Array(Set(currentConnections.map {$0.metadata.sourceIP}))
        currentApplications = Array(Set(currentConnections.compactMap {$0.metadata.pid}).compactMap({[weak self] in self?.applicationMap[$0]}))
    }
